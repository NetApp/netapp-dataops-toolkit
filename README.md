...
The following Python libraries that are not generally bundled with a standard Python installation are required in order for the NetApp Data Science Toolkit to function correctly - netapp-ontap, pandas, tabulate, requests, boto3. These libraries can be installed with a Python package manager like pip.
...
```py
from ntap_dsutil import cloneVolume, createVolume, deleteVolume, listVolumes, mountVolume, createSnapshot, deleteSnapshot, listSnapshots, restoreSnapshot, listCloudSyncRelationships, syncCloudSyncRelationship, listSnapMirrorRelationships, syncSnapMirrorRelationship
```
