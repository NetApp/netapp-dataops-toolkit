...
server2_name="b-1-w2"
model_pvc="model-repo-test"
command="netapp_dataops_k8s_cli.py create triton-server --server-name=$server2_name --model-repo-pvc-name=$model_pvc --image=nvcr.io/nvidia/tritonserver:21.11-py3 --memory=512Mi --cpu=0.25 --allocate-resource=nvidia.com/mig-1g.5gb=1"
echo "Running: $command"
eval $command
printf "\n"
...
model_pvc="model-repo-test1"
namespace="dsk-test"
command="netapp_dataops_k8s_cli.py create triton-server --namespace=$namespace --server-name=$server4_name --model-repo-pvc-name=$model_pvc --image=nvcr.io/nvidia/tritonserver:22.03-py3 --memory=512Mi --cpu=0.25 --nvidia-gpu=1 --load-balancer --allocate-resource=nvidia.com/mig-1g.5gb=1"
echo "Running: $command"
eval $command
printf "\n"
...
model_pvc="model-repo-test1"
namespace="dsk-test"
command="netapp_dataops_k8s_cli.py create triton-server -n $namespace -s $server6_name -v $model_pvc -i nvcr.io/nvidia/tritonserver:22.03-py3 -m 512Mi -p 0.25 -g 1 -b -r nvidia.com/mig-1g.5gb=1"
echo "Running: $command"
eval $command
printf "\n"
